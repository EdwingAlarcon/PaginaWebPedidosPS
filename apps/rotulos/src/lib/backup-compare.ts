...
function rowKey(table: BackupTableName, row: unknown, index: number): string {
...
export function compareBackupSnapshots(before: FullBackupPayload, current: FullBackupPayload): BackupCompareReport {
  ...
  for (const table of TABLES) {
    const beforeRows = new Map(tableRows(before, table).map((row, index) => [rowKey(table, row, index), stableStringify(row)]));
    const currentRows = new Map(tableRows(current, table).map((row, index) => [rowKey(table, row, index), stableStringify(row)]));
