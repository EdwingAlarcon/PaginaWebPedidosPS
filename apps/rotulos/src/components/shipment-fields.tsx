...
export function ShipmentFields({ value, onChange, errors, allowManualEdit }: { value: LabelDraft; onChange: (value: LabelDraft) => void; errors: Record<string, string>; allowManualEdit: boolean }) {
  ...
  return (
    <fieldset className="form-section">
      ...
      <label className="field"><span>Transportadora</span><input value={value.carrier} aria-describedby={errors.carrier ? "carrier-error" : undefined} onChange={(event) => set("carrier", event.target.value)} />{errors.carrier ? <small id="carrier-error">{errors.carrier}</small> : null}</label>
